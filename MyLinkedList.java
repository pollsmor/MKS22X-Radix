public class MyLinkedList<E> {
  private int m_size;
  private Node<E> m_start;
  private Node<E> m_end;
  ...
  private Node<E> getNthNode(int index) {
    ...
    Node<E> current = m_start; //Start from start and loop through nodes
    int i = 0;
  ...
  public boolean add(E value) {
    if (m_size == 0) { //If it's an empty list I have to put in a start
      m_start = new Node<E>(value, null, null); //One item, nothing to left or right
      m_end = m_start; //Originally I made a new Node here. Thanks Ethan for tipping me off on this!
    }

    else {
      m_end.setNext(new Node<E>(value, null, m_end));
  ...
  public E set(int index, E value) {
    ...
    E temp = getNthNode(index).getData(); //Get nth node, so that I can run Node.getData() on it and return it later

    getNthNode(index).setData(value); //Gets nth node, so that I can run Node.setData(E value) on it
  ...
  public int indexOf(E value) { //similar to contains() but returns an index
    for (int i = 0; i < m_size; ++i)
  ...
  public void add(int index, E value) {
    ...
    if (m_size == 0) { //from boolean add()
      m_start = new Node<E>(value, null, null);
      m_end = m_start;
    }

    else if (index == 0) { //Adding to before the list's first element
      m_start.setPrev(new Node<E>(value, m_start, null));
      ...
      Node<E> temp = getNthNode(index); //suggestion from class to only run this once
      temp.prev().setNext(new Node<E>(value, temp, temp.prev()));
  ...
  public E remove(int index) {
    ...
    if (index == 0)
      ...
      Node<E> tempNode = getNthNode(index); //suggestion from class
